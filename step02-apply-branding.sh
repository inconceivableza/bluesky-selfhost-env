...
for repoDir in $repoDirs
  do
    (
      ...
      true # $script_dir/autobranch.sh work $src_branch dockerbuild local-rebranding-$REBRANDING_NAME $src_branch main
...
if [ "$REBRANDING_DISABLED" == "true" ]
  then
    ...
    REBRANDED_TARGET_REPOS="$(for f in $REBRANDED_REPOS; do [ "${TARGET_REPOS/$f/}" != "${TARGET_REPOS}" ] && echo $f ; done)"
    if [ "$REBRANDED_TARGET_REPOS" != "" ]
      then
        show_heading "Rebranding repos: $REBRANDED_TARGET_REPOS" "by scripted changes"
        exit 1
        ...
        for branded_repo in $REBRANDED_TARGET_REPOS
          do
            (
        ...
        show_warning "Not rebranding" "as none of the target repositories are in REBRANDED_REPOS"
    fi
  fi
