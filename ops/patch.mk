...
patch-dockerbuild: ${rDir}/feed-generator/.dockerbuild
...
${rDir}/feed-generator/.dockerbuild:
	...
	(cd ${rDir}/feed-generator; git status; git checkout ${branch2patch} $(checkoutbranchargs) dockerbuild )
	...
	touch $@
	(cd ${rDir}/feed-generator; git add . ; git commit $(commitargs) -m "update: dockerbuild"; )

${rDir}/ozone/.dockerbuild:
	@echo "make branch and applying patch..."
	(cd ${rDir}/ozone git status; git checkout ${branch2patch} $(checkoutbranchargs) dockerbuild )
	for ops in `ls ${wDir}/patching/1*.sh | grep ozone`; do wDir=${wDir} rDir=${rDir} pDir=${wDir}/patching DOMAIN=${DOMAIN} asof=${asof}  $${ops} ; done
	touch $@
	(cd ${rDir}/ozone; git add . ; git commit $(commitargs) -m "update: dockerbuild"; )

${rDir}/indigo/.dockerbuild:
	@echo "make branch and applying patch..."
	(cd ${rDir}/indigo; git status; git checkout ${branch2patch} $(checkoutbranchargs) dockerbuild )
...
${rDir}/social-app/.dockerbuild:
	@echo "make branch and applying patch..."
	(cd ${rDir}/social-app; git status; git checkout ${branch2patch} $(checkoutbranchargs) dockerbuild )
	...
	touch $@
	(cd ${rDir}/social-app; git add . ; git commit $(commitargs) -m "update: dockerbuild"; )

${rDir}/jetstream/.dockerbuild:
	@echo "make branch and applying patch..."
	(cd ${rDir}/jetstream; git status; git checkout ${branch2patch} $(checkoutbranchargs) dockerbuild )
	...
	touch $@
	(cd ${rDir}/jetstream; git add . ; git commit $(commitargs) -m "update: dockerbuild"; )

_patch-selfhost-even-not-mandatory: ${rDir}/social-app/.selfhost-${DOMAIN}
${rDir}/social-app/.selfhost-${DOMAIN}::
	@echo "make branch and applying patch..."
	(cd ${rDir}/social-app; git status; git checkout ${branch2patch} $(checkoutbranchargs) selfhost-${asof}${DOMAIN} )
	...
	touch $@
	(cd ${rDir}/social-app; git add . ; git commit $(commitargs) -m "update: selfhosting domain: ${DOMAIN} asof: ${asof}"; )
